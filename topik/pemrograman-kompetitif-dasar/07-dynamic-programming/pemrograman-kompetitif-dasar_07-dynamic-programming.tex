...
\begin{itemize}
  ...
  \item Diberikan nominal-nominal koin yang tersedia dan nominal uang yang ingin anda bayar. Asumsikan kita memiliki tak hingga koin untuk setiap nominal koin yang ada. Tentukan berapa banyaknya minimal koin yang harus kita berikan untuk membayar tepat uang yang ingin anda bayar.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item Kita dapat melakukan \newTerm{memoisasi}, dimana kita menyimpan hasil nilai $f(x)$ setelah menghitung nilai $f(x)$ agar jika kita memerlukan nilai $f(x)$ pada masa depannya, kita tidak perlu menghitungnya kembali.
...
\begin{itemize}
  \item Diberikan $N$ buah barang. Barang ke-$i$ memiliki harga $v_i$ rupiah dan memiliki berat $w_i$ gram. Anda memiliki tas yang berkapasitas $W$ gram. 
  \item Anda ingin memasukan beberapa barang kedalam tas sedemikian sehingga jumlah berat dari barang-barang yang anda masukan tidak lebih dari kapasitas tas dan jumlah harga dari barang-barang yang anda masukan sebanyak mungkin.
\end{itemize}
\end{frame}
...
\begin{itemize}
  \item Anda harus memotong-motong sebuah batang kayu dengan panjang $L$ meter pada $N$ titik menjadi $N+1$ bagian. Titik ke-$i$ berada di $L_i$ meter dari sebuah ujung.
  \item Untuk memotong kayu, anda memerlukan usaha sebanyak panjang kayu yang sedang anda potong.
